*---------------------------------------------------------------*;
* %make_define2 creates the define.xml file for the SDTM and ADaM.  
...
%macro make_define2(path=,metadata=);
...
data define_header;
    ...
    put @1 '<?xml version="1.0" encoding="ISO-8859-1" ?>' /
        ...
        @1 '<!-- Created by the %make_define2 SAS macro                                          -->' /
        @1 '<!-- ******************************************************************************* -->' /
        @1 '<ODM' /
...
data variable_metadata;
    ...
    merge variable_metadata (in = in_vm rename=(domain = oid))
          vlmid             (in = in_vlm rename=(domain = oid))
          ;
      by oid variable;
...
data valuelevel whereclause2 (keep = whereclauseoid domain valuevar valuename);
...
data itemgroupdef;
    length label $ 40;
    merge toc_metadata (rename=(commentoid=dmcommentoid))
          VARIABLE_METADATA(drop=label)
          ;
    by oid;
